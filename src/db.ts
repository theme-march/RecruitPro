  ...
  try {
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS users (
        ...
        role ENUM('super_admin', 'admin', 'agent', 'accountant', 'data_entry') NOT NULL,
    ...
    // Packages table (predefined package amounts)
    await connection.query(`
    ...
    if ((packageRows as any[])[0].cnt === 0) {
      ...
      await connection.query(`INSERT INTO packages (name, amount, description) VALUES
        ('Basic', 1000.00, 'Entry level package'),
        ('Standard', 2500.00, 'Most popular package'),
        ('Premium', 5000.00, 'Top tier package with extras')
      `);
    }
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS payments (
        ...
        amount DECIMAL(10,2) NOT NULL,
        payment_type ENUM('visa', 'medical', 'ticket', 'service') NOT NULL,
        payment_method ENUM('cash', 'sslcommerz') NOT NULL,
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS ssl_transactions (
        ...
        tran_id VARCHAR(255) UNIQUE NOT NULL,
